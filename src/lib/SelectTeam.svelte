<script lang="ts">
    ...
    let { game_id, value = $bindable(), is_blue } =
...
{#if typeof(teams) === 'undefined'}
    <NiceErrorTextBox error={error} bind:input={team_string}></NiceErrorTextBox>
{:else}
    <select bind:value={value}>
        {#each teams as team}
            <option value={team}>{format_team(team.team, team.is_ab_team)}</option>
        {/each}
    </select>
{/if}