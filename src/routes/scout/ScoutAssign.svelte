...
<script lang="ts">
    ...
    import type { Game, GameTeamDataScouter, MatchData, ScouterInsertFront } from '../../lib/schema/types.gen';
   ...
   function format_scout_pick_data(): ScouterInsertFront {
    ...
    const final_player_indexs = Array
        .from(player_indexs.entries())
        .sort((a, b) => a[1] - b[1])
        .map(([name]) => name);
...
<div style="background-color: #2e8b57; padding: 15px; margin: 10px; border-radius: 10px;">
    <h3>Quick Assign by Team Position</h3>
    <p style="font-size: 0.9em; margin-bottom: 10px;">Assign scouters to team positions (1st team, 2nd team, etc.) across all games</p>
    ...
    <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #fff;">
        <div style="display: flex; gap: 10px; align-items: center; margin-bottom: 8px;">
            ...
            <UserSelector 
                bind:value={masterRedMvp}
            />
        </div>
        <div style="display: flex; gap: 10px; align-items: center; margin-bottom: 8px;">
            <span style="min-width: 100px;">Blue MVP:</span>
            <UserSelector 
                bind:value={masterBlueMvp}
            />
        </div>
    </div>
...
{#if typeof(all_games) === 'string'} 
    {all_games}
{:else}
    <FormWithLoading stop={!stop} dispatch={dispatch}>
            ...
            {#each game.teams as team, teamIndex}
                <div>
                    ...
                    {#each scouters.get(team.id) ?? [] as scouter, i}
                        <UserSelector 
                            value={scouter} 
                            onchange={(value) => updateScouter(team.id, i, value)} 
                        />
                    {/each}
                    <button type="button" onclick={() => addScouter(team.id)}>+</button>
                </div>
            {/each}
